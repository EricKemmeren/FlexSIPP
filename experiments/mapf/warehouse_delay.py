...
def run_flexsipp_scenario(location_file, scenario_file):
    ...
    result = flexSIPP.run_search(rerouting_agent.origin.name, rerouting_agent.destination.name, start_time, graph.global_end_time, optimize_total_delay=False, redirect_stderr=os.path.join("output", "stderr_warehouse_FlexSIPP.txt"), write_fsipp_graph=os.path.join("output", "warehouse_SIPP.txt"))
    ...
    atf, new_route, minimum_delays, _ = result.get_fastest_route(rerouting_agent, original_arrival_time_reroute, actual_departure_time, agents, discrete=True, optimize_total_delay=False, print_agent_delays=True)
