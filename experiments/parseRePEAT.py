def parse_repeat(s):
    #s is a string with the text output of a repeat search this parses it into the compound atf, and the individual augmented SIPP plans for each segment
    s = s.strip("\n").split("\n") # avoid empty element in list
    return parse_list_of_outputs_macos(s)
...
def parse_list_of_outputs_macos(s):
    # s is the output split on newline characters
    i = 0
    while "Nodes generated" not in s[i]:
        i += 1
