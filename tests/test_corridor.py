...
class TestCorridorExample(unittest.TestCase):
    ...
    def test_corridor_example(self):
        ...
        print(f"Agent {rerouting_agent1} is delayed at time {start_time} and has new path {'-'.join([node[0].name for node in res2['route'] if isinstance(node[0], Node)])} with atf {atf2} that delays agent 2 with route {'-'.join([node.name for node in agents[2].route if isinstance(node, Node)])} at node (1,1) for [0,1]")
        ...
        actual_delay1 = 1
        res_atf1, new_route1, minimum_delays1 = result1.get_fastest_route(float(actual_delay1), agents, discrete=True)
        ...
        # TODO this finds an illegal route
        flexSIPP2._write(open(os.path.join(os.path.dirname(os.path.dirname(__file__)), "corridor2.txt"), "w"))
        ...
        self.assertEqual(atf2[2], 7) # beta

        # TODO why is 3,0 in delay nodes for agent 3, which doesn't use this
        self.assertDictEqual(res2["delays"], {1: [], 2: [], 3: [(graph.nodes["(0,1)"], 0, 0, 5), (graph.nodes["(3,0)"], 0, 0, 5)], 4: [], 5: []})
        ...
        self.assertEqual(result2.unique_path_eatfs["(0,0)->(0,1)->(1,1)->(2,1)->(3,1)->(3,0)"][1][2], 7)
        ###################################
        for x, node in graph.nodes.items():
            print(f"Node {x} has unsafe intervals {' '.join([f'A{ui.by_agent}: <{ui.start},{ui.end}> [{ui.duration}] recovery {ui.local_recovery_time}'for ui in node.unsafe_intervals])}")

        actual_delay2 = 4
        res_atf2, new_route2, minimum_delays2 = result2.get_fastest_route(float(actual_delay2), agents, discrete=True)
        ...
        self.assertEqual(res_atf2[2], 7)
        self.assertIn(graph.nodes["(0,1)"], minimum_delays2[agents[3]])
        self.assertIn(graph.nodes["(0,2)"], minimum_delays2[agents[3]])
        print(f"Agent {rerouting_agent2} is delayed at time {actual_delay2} and has new path {'-'.join([node[0].name for node in new_route2 if isinstance(node[0], Node)])} with atf {res_atf2} that delays agent 3 {[(str(x), y) for x,y in minimum_delays2[agents[3]].items()]}")
        ...
        self.assertEqual(graph.nodes["(3,1)"].unsafe_intervals[3].end, 10)
        ###################################
        ...
        ##### Test Second FlexSIPP run #####
        self.assertEqual(len(result3.found_routes), 2)
        atf1, res1 = result3.found_routes[0]
        ...
        actual_delay3 = 4
        res_atf3, new_route3, minimum_delays3 = result3.get_fastest_route(float(actual_delay3), agents, discrete=True)
        ...
        for agent, flexibility_used in minimum_delays3.items():
