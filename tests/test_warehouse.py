...
class TestWarehouseExample(unittest.TestCase):
    ...
    def test_warehouse_example(self):
        ...
        graph, agents = create_mapf_instance_from_paths(
            os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "mapf", "warehouse", "warehouse.map"),
            os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "mapf", "warehouse", "paths.txt"),
            end_time)
